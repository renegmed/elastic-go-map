 ...
 */
// match geodata consumer -i 33324 -d 2000 -u km

var combinedGeoDataCmd = &cobra.Command{
	Use:   "geodata",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("%v\n", geoData)
		out, err := os.Create("geoData.json")
		if err != nil {
			fmt.Println(err)
			return
		}
...
func init() {
	geodataCmd.AddCommand(combinedGeoDataCmd)
	combinedGeoDataCmd.Flags().StringP("consumer", "i", "", "consumer ID")
	combinedGeoDataCmd.Flags().StringP("distance", "d", "", "maximum distance from consumer")
	combinedGeoDataCmd.Flags().StringP("unit", "u", "", "unit of distance, e.g. km, mi")
}
